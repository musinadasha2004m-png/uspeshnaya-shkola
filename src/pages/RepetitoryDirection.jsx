...
// 72°, старт сверху, один общий радиус) вокруг пустого центра — по
// той же схеме, что иконки вокруг фото мальчика в Hero на главной,
// только без фото и на полную окружность (там нижняя дуга оставалась
// пустой под силуэт).
const HERO_ICONS = [
  { src: iconRuler, left: '50%', top: '14%', rotate: -6, delay: 0, duration: 4.6 },
  { src: iconFlask, left: '84.2%', top: '38.9%', rotate: 8, delay: 0.5, duration: 5 },
  { src: iconGlobe, left: '71.2%', top: '79.1%', rotate: -8, delay: 1, duration: 4.8 },
  { src: iconAtom, left: '28.8%', top: '79.1%', rotate: 10, delay: 1.5, duration: 5.2 },
  { src: iconBiology, left: '15.8%', top: '38.9%', rotate: -10, delay: 0.8, duration: 4.4 },
]
    ...
    <div className="min-h-screen bg-bg-white">
      ...
      <section className="bg-bg-white">
        <Container className="flex flex-col gap-10 py-10 md:py-14 xl:flex-row xl:items-center xl:gap-10">
          ...
          {/* Иконки предметов по кругу — пустой центр, мягкая подложка,
              пара маленьких звёзд-акцентов на своих подложках рядом.
              На мобильных (< sm) скрыта полностью (display: none) — под
              текстом ей не место, тексту отдаётся вся ширина; с sm и
              шире уходит под текст (не помещается рядом без переноса
              заголовка на 3 строки); с xl (1280px+) занимает весь
              остаток строки и центрируется в нём (а не в узкой
              колонке), поэтому оказывается ровно посередине свободного
              места справа от текста, на уровне (по вертикали)
              текстового блока. */}
          <div className="hidden sm:block xl:flex-1">
            <div className="relative mx-auto aspect-square w-full max-w-[380px] sm:max-w-[460px] xl:max-w-[440px]">
                ...
                >
                  ...
                  >
                    <img
                      ...
                      className="h-auto w-[96px] max-w-none drop-shadow-[0_8px_16px_rgba(27,36,85,0.12)] sm:w-[112px] lg:w-[132px] xl:w-[120px]"
                    />
                  </div>
                </div>
              ))}

              <span className="absolute -right-4 -top-4 flex h-12 w-12 items-center justify-center rounded-full bg-brand-green/15">
                <img src={iconStar} alt="" className="h-7 w-7 object-contain" />
              </span>
              <span className="absolute -bottom-3 -left-5 flex h-9 w-9 items-center justify-center rounded-full bg-brand-green/15">
      ...
      <section className="bg-bg-lavender">
        <Container className="py-10 md:py-14">
          ...
          <div className="mt-6 flex flex-col items-center gap-8 md:mt-8 xl:flex-row xl:items-center xl:justify-between xl:gap-10">
            <div className="w-full max-w-4xl rounded-card bg-white p-5 shadow-card md:p-8">
            ...
            {/* Книга + пара звёзд — перенесены из Hero, здесь книга
                заметно крупнее, каждая иконка на своей бледно-зелёной
            ...
            <div className="relative h-40 w-40 shrink-0 sm:h-44 sm:w-44 xl:h-48 xl:w-48">
              <div className="flex h-full w-full items-center justify-center rounded-full bg-brand-green/10">
                <img
                  src={iconLiterature}
                  alt=""
                  className="h-28 w-28 object-contain sm:h-32 sm:w-32 xl:h-36 xl:w-36"
                />
              </div>
              <span className="absolute -right-3 -top-3 flex h-11 w-11 items-center justify-center rounded-full bg-brand-green/15">
                <img src={iconStar} alt="" className="h-7 w-7 object-contain" />
              </span>
